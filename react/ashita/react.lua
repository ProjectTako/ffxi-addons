...
ashita.register_event('command', function(cmd, nType)
    ...
    if (args[1] == "/react") then
      ...
      elseif (args[2]:lower() == 'safemode') then
        if (args[3]:lower() == 'on') then
          safemode = true;
          print("\31\200[\31\05React\31\200]\31\207 " .. "Safemode on, will not move if Terror, Stun or Petri.");
        elseif (args[3]:lower() == 'off') then
          safemode = false;
          print("\31\200[\31\05React\31\200]\31\207 " .. "Safemode off, will move no matter what.");
        end
      end
    end
...
function process_reaction(reaction, action_name, actor, target)
  if (reaction ~= nil) then
    ...
    if (reaction == 'turnaround' or reaction == 'facemob') then
      --Make sure the player isn't dead before reacting
      if AshitaCore:GetDataManager():GetParty():GetMemberCurrentHP(0) > 0 then
        if not safemode or not (HaveBuff("Petrification") or HaveBuff("Terror") or HaveBuff("Stun")) then
